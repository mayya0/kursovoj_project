...
double linear_search(double (*func)(double), double target, double* values, int N);  // линейный поиск
...
int main() {
    ...
    while (1) {
        ...
        case 1:
        {
            double min_value = find_min(func, values, (choice == 1) ? 1 : n);
            double max_value = find_max(func, values, (choice == 1) ? 1 : n);
        ...
        case 2:
        {
            ...
            scanf("%lf", &target);
            double result = linear_search(func, target, values, (choice == 1) ? 1 : n);
            if (!isnan(result)) {
...
double* mass_shag(double xmin, double dx) {
    double* mass = (double*)malloc(n * sizeof(double)); // выделение памяти для массива значений
...
int write_to_file(const char* filename, double* values, int count, double (*func)(double)) {
    ...
    if (outputFile == NULL) { // проверка успешности открытия файла
...
double linear_search(double (*func)(double), double target, double* values, int N) {
    for (int i = 0; i < N; i++) {
        if (fabs(func(values[i]) - target) < 1e-2) {  // увеличена погрешность для более точного поиска
            return values[i];
        }
    }
